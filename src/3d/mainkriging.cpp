...
bool getNormalizedPositions( double* rbfCenterPositions, double* xk, double* yk, double* zk, int rbfWidth, int gridSize, int numCenters, int interpFuncExtent, int numFreq )
{
	...
	{
		//double scale = numFreq/(double)(numFreq+2*interpFuncExtent);
		double scale = 1;
		for( i=0; i<numCenters; i++ )
		{
...
if(myid == 0){
	...
	if( argc < 15 )
    {
		printf("\n\nUsage : %s alpha interpFuncExtent numFreq gridSize inputFile1(coords) inputFile2(coefs) outputFileName kifmmoptions (ftilde+1) (f+1) GMRES_tolerance [bitmap] Target distance-criterion-cte minlevel \n",argv[0]);
...

	{
		...
		
		{
			if( fscanf( fp, "%lf %lf %lf\n",
                          &(rbfTargetCenterPositions[0*3+0]), &(rbfTargetCenterPositions[0*3+1]), &(rbfTargetCenterPositions[0*3+2]) ) != 3 )
			{
				printf("Format error in file %s line %d\n", inputFile, 0+1 );
				fftw_free( RBF3D );
		...
		{
			if( fscanf( fp, "%lf %lf %lf\n", 
                          &(rbfTargetCenterPositions[i*3+0]), &(rbfTargetCenterPositions[i*3+1]), &(rbfTargetCenterPositions[i*3+2]) ) != 3 )
			{
				printf("Format error in file %s line %d\n", inputFile, i+1 );
				fftw_free( RBF3D );
			        delete [] rbfCenterValues;
	...
	printf("Finished inputing source and target files. Wall-clock time used: %f seconds \n",dsecnd() - t0);
       	...
       	// Vanishing moments for 3D
	int nnei = van*(van+1)*(van+2)/6;
	// int nnei = van*(van+1)/2; // For 2D
	...
	ierr = gmres_init(argc, argv, n, numCenters, gridSize, alpha, d, interpFuncExtent, tol, max_iter, van, 
			  pvan, rbfCenterPositions,  RBF3D, solver, mA, mD, minlevel, rhs);
	t1 = dsecnd();
