...
FWTd3::FWTd3(double** coord, int N, int van, int pvan, int d, int minlevel){
	...
	printf("Min Level %d \n",minKwlevel);
        ...
        int pnnei = pvan*(pvan+1)*(pvan+2)/6;
...
void FWTd3::trafo(double** K1){
	...
	// This step requres O(n^2) memory. This is odd.
...
void FWTd3::trafo_diag_blk(Diag_Blk* db){
	...
	for (li = wavMaxLevel; li >= minl-1; li--){
		...
		hb->sortAvailableHB(li);
			...
			for(kk = 0; kk < hb->neighborHBLength[si]; kk++){
				int sk = hb->neighborHBNodes[si][kk];
				//printf("Treemark %d si %d Length %d kk % d sk %d \n",treemark, si, hb->neighborHBLength[si], kk, sk);  	
				if (hb->support[sk][0] < 0)
					//sk is in the finest cube,
					//which should contain only 1 point
					hb->mergeSupport(t,tlen,&sk,1);
			...
			if(hb->neighborHBLength[si] != tlen){
			  // printf("Check neighborHBLength (%d) vs. tlen(%d)\n",hb->neighborHBLength[si],tlen);///
				exit(-1);
			}///
			//loop through the support of si
			...
			for(jj = 0; jj < hb->blength[si]; jj++){
				...
				for(kk = 0; kk < tlen; kk++){
				  ...
				  if(!hb->testAvailableHB(sk,sj,li)){
				    printf("Something wrong with testAvailableHB: (%d, %d), Treemark %d kk %d tlen %d. Increase ptilde or decrease tau \n",sk,sj,treemark, kk, tlen);
						exit(-1);
					}
		...
		if(li < db->sparse_level && db->blk_size[li] < 1e4){
			...
			for(ii = 0 ; ii < blk_n; ii++){
				for(jj = 0; jj < blk_n; jj++)
					blk[ii + blk_n*jj] = hb->highGet(hb2high[hb->waveletNodes[hb->wavlist[ii+hb->wavlevels[li][0]]]],
									 hb2high[hb->waveletNodes[hb->wavlist[jj+hb->wavlevels[li][0]]]],li);
			}
	...
	if(coarseLifted){
		...
		for(ii = 0 ; ii < blk_n; ii++){
			for(jj = 0; jj < blk_n; jj++)
				blk[ii + blk_n*jj] = hb->highGet(hb2high[hb->waveletNodes[hb->wavlist[ii+hb->wavlevels[li][0]]]],
								 hb2high[hb->waveletNodes[hb->wavlist[jj+hb->wavlevels[li][0]]]],minl-1);
		}
		for(ii = 0; ii < blk_n*blk_n; ii++)
			blk_lu[ii] = blk[ii];
		lu_init(blk_n,blk_n,blk_lu,ipiv);
...
void FWTd3::multilevel_trafo_diag_blk(Diag_Blk* db){
	...
	delete[] FMMtemp; //temparory memory for FMMevalHB
	cout << "trafo_diag_blk: Done. " << '\n';
//	writeIntMatrix("/work/utexas/ti/junli/hb2high.dat",1,hb->size,&(hb2high));///
}
...
void FWTd3::multiLift(int cube,int van){
	...
	if(nei->tree[cube][0] == nei->tree[cube][1]){
		...
		//  printf("   Accessing vande3->vandemonde3(&(nei->coord[%d]),van,1)\n",-pt-1);///
...
//tindex: pointers from node index to tsupp index
	...
	int** tree = nei->tree;
	nei->getQuadrant3(bound[node][0],bound[node][1],bound[node][2],bound[node][3],bound[node][4],bound[node][5],tree[node][0],tree[node][1],y);
	tlength = y.size();
	...
	for(int i = 0; i < hb->size; i++){
		...
		for(int ii = -d; ii < d+1; ii++)
		  for(int jj = -d; jj < d+1; jj++)
			...
			// printf("bounds: [%f, %f, %f] - [%f, %f, %f]\n",bound1[0], bound1[2], bound1[4],bound1[1],bound1[3],bound1[5]);///
			treemark = 0;
		...
		// printf("**Cube %d Length %d Level %d \n",cube,length,level);
...
// Test neighborhood
  ...
  printf("Tests ----------------------------------------------------------\n");
	...
	for(int si = 0; si < hb->size; si++){
		...
		// multileveldistcritUpper(level, si, nodes, nodeslength);
		// MultilevelNeighbor(level, si, nodes, nodeslength);
		// printf("Node %d Level %d Length %d Second Length %d \n",si,level,length,*nodeslength);

		// First Test (Ok)
		if (!(length==*nodeslength)) printf("First Test: Test broke Node %d Level %d Cube %d Original Length %d Second Length %d \n",
		...
		for(int j = 0; j < length; j++) {
		  ...
		  if (!(node == nodes[j])) printf("Second Test: Test broke j %d Cube %d Current Node %d Level %d Original Length %d Second Length %d Original Node %d Comparison Node %d  \n", j, cube, si, level, length, *nodeslength, node, nodes[j]);
		  flag = 1;
		}
	}
}
	...
	for(int i = 0; i < hb->size; i++){
		...
		for(int ii = -d; ii < d+1; ii++)
		  for(int jj = -d; jj < d+1; jj++)
			...
			bound1[4] = bound[4] + stepZ*kk + eps;
			bound1[5] = bound[5] + stepZ*kk - eps;
			// printf("bounds: [%f, %f, %f] - [%f, %f, %f]\n",bound1[0], bound1[2], bound[4],bound1[1],bound1[3],bound1[5]);///
			treemark = 0;
			// efficiently locate the neighboring node
			int found = 0;
			while(nei->tree[treemark][11] < level){
...
void FWTd3::findMultiLevelNeighborHBNodesT(int d){
	...
	for(int i = 0; i < hb->size; i++){
	    ...
	    while(multilevelcounter > -1) {
	      ...
	      // printf("Initial Nodes, Treemark %d Level %d Multilevel  %d \n", treemark, level, multilevel);
	    ...
	    for(int f = 0; f < hb->multilevelcount; f++){
	      ...
	      // printf("%d ", hb->MutliLevelneighborHBNodesTranspose[i][f]);
	    }
...
void FWTd3::StoreLinkDataMultiLevel(int level, int multilevel, int i, int MAX_LEVEL, int counter, int *levelcubes, int *nodes){
  ...
  int valueq = 0;
		...
		if (nei->tree[hb->cubenode[hb->treelink[levelcubes[q]][f]]][11] == multilevel) {
		  ...
		  // printf("%d ",nodes[totalcounter]);
		  // if(hb->wavelet[nodes[totalcounter]]){ 
		  //	printf("sk = %d Wavelet index = %d \n", nodes[totalcounter], hb->waveletIndex[nodes[totalcounter]] );
		  //  }
		  totalcounter++;
		}
	      }
	    ...
	    // printf("\n");
	    // printf("Total Counter %d Multilevel %d \n", totalcounter, multilevel);
...
void FWTd3::multileveldistcritUpper(int level, int currentnode, int *nodes, int *nodeslength){
            ...
            // printf("MultiLevelNeighborhoodNodes Current Node %d Length %d \n",currentnode,hb->MutliLevelneighborHBNodesLength[currentnode]);
	 ...
	 while (treemark > -1){
	   if (nei->tree[treemark][11] == li) {
	     ...
	     if(hb->linklength[treemark]>nnei){
	       ...
	       // Debugging
	       /*
	       printf("Distance criterion Level j %d Node %d Nodes Length %d \n", lj, si, *nodeslength);
		 ...
		 // printf("treemark %d si %d Length %d kk % d sk %d \n",treemark, si, hb->neighborCubesLength[treemark], kk, sk);
		   if (sk == -1 ) {
		     printf("Error ********************************************* \n",sk);
		     exit(-1);}
		 if (hb->support[sk][0] < 0)
		 ...
		 // Alternatively, we could get the support from neighboring cubes
	       ...
	       // loop through the support of si
	       // printf("Treemark %d Level %d Blength %d tlen %d \n", treemark,  nei->tree[treemark][11], hb->blength[si], tlen);
	       for(jj = 0; jj < hb->blength[si]; jj++){
		 int sj = hb->support[si][jj];
		 FMMcount = 0;
		 ...
		 for(kk = 0; kk < tlen; kk++){
		   ...
		   int sk = t[kk];
		   // None is available
		   // if(hb->testAvailableHB(sj,sk,    li + wavMaxLevel*(wavMaxLevel - lj)   ) ){
		   //  temp[jj][kk] = hb->highGet(hb2high[sj],hb2high[sk],li + wavMaxLevel*(wavMaxLevel - lj));
		   // }
		   ...
		   if(hb->testAvailableHB(sj,sk, li + 1 + q*(wavMaxLevel + 1) - (q-1)*q/2)){
		     temp[jj][kk] = hb->highGet(hb2high[sj],hb2high[sk], li + 1 + q*(wavMaxLevel + 1) - (q-1)*q/2);
		       // printf("Temp %f High %f \n",temp[jj][kk],hb->highGet(hb2high[sj],hb2high[sk], li + 1));
		     }
		   else{
		     // temp[jj][kk] = 0;
		     // printf("Something wrong  li %d, lj %d, si %d block index %d treemark % d \n",li,lj,si,li + q*(wavMaxLevel + 1) - (q-1)*q/2, treemark);
		     // exit(-1);
		     //record the position to use FMMevalHB
		     FMMkk[FMMcount] = kk;
		     FMMsj[FMMcount] = sk;
		 ...
		 if(FMMcount>0)
		   ...
		   FMMevalHB(hb,FMMsj,FMMcount,sj,eval,FMMtemp,nei,dist,rout,site);
	         ...
	         for(kk = 0; kk < FMMcount; kk++){				  
		   temp[jj][FMMkk[kk]] = eval[kk];
		   // hb->highSet(hb2high[sj],hb2high[FMMsj[kk]],eval[kk]); //not working unless sortAvailableHB immediately after
		   // if(abs(eval[kk]) < 1e-14) 
		   // printf("eval[%d,%d] = %lf  %f   \n",sj,FMMsj[kk],eval[kk],temp[jj][FMMkk[kk]] );///
		 }
	       }
	       // compute scaling functions and wavelets in the cube
		 ...
		 for(ll = 0; ll < tlen; ll++){
		   ...
		   // printf("Cols: highset ...\n");
		   // printf("%d eval[%d,%d] = val %f \n",ll,sj,sl, val);
		   hb->highSet(hb2high[sj],hb2high[sl], li + q*(wavMaxLevel + 1) - (q-1)*q/2, val);
		   // printf("Cols: trackAvailableHB ...\n");///
		   // printf("%d eval[%d,%d] = matrix %f val %f Test Available %d Level %d \n",ll,sj,sl, hb->highGet(hb2high[sj],hb2high[sl],li),val, hb->testAvailableHB(sj,sl, li), li);
		   // printf("Matrix %f \n",hb->highGet(hb2high[sj],hb2high[sl],li));
		 }
	       }
	     }
	   }
	 ...
	 while (treemark > -1){
	   if (nei->tree[treemark][11] == li) {
		 ...
		 for(ll = 0; ll < tlen; ll++){
		   ...
		   hb->highSet(hb2high[sj],hb2high[sl], li + q*(wavMaxLevel + 1) - (q-1)*q/2, val);
		   // printf("Cols: trackAvailableHB ...\n");///
		   // printf("%d eval[%d,%d] = matrix %f val %f \n",ll,sj,sl, hb->highGet(hb2high[sj],hb2high[sl],lj), val);
		 }
	       }
	     }
	   }
...
void FWTd3::Kwblockmatrixrow(int li, int lj, int currenttremark, int nnei, double **temp, int *FMMkk, int *FMMsj, double *eval, int *FMMtemp, int *t, int *nodes, int *nodeslength, int wavMaxLevel){
		...
		// printf("Secondary Currenttremark %d li %d  Current Level %d \n",currenttremark,li,nei->tree[currenttremark][11]);
		  ...
		  if(hb->linklength[currenttremark]>nnei){
			...
			out = 1;
			// printf("**** lj %d %d treemark %d \n",lj,hb->MutliLevelneighborHBNodesTranspose[si][2*i], treemark);
			break;
		      }
		    }
		    ...
		    for(jj = 0; jj < hb->linklength[currenttremark]; jj++){
			  ...
			  //  Debug
			     if (li == lj){
			       // printf("*treemark %d row %d sl %d sj %d li %d lj %d duallevel %d val %f \n", currenttremark, 
			       // ll, sl, sj, li, lj, (li + q*(wavMaxLevel + 1) - (q-1)*q/2 ),  val );
			    }
		       }
		       //The neighbors of sj is the same as those of si
		       //The points in t and neighborHBNodes are for the same
	 ...
	 double* rout   = new double[nsite];          // spline evaluation results
		...
		// printf("Secondary Currenttremark %d li %d  Current Level %d \n",currenttremark,li,nei->tree[currenttremark][11]);
		  ...
		  if(hb->linklength[currenttremark]>nnei){
		    ...
		    printf("Colmn si = %d \n", si);
		    // multileveldistcritUpper(li, si, nodes, nodeslength);

		    for(int i = 0; i < hb->MutliLevelneighborHBNodesLengthTranspose[si]; i++){
		      // printf("% d", hb->MutliLevelneighborHBNodesTranspose[si][i]);
		    }
		    // printf("\n");
			...
			out = 1;
			treemark = hb->MutliLevelneighborHBNodesTranspose[si][2*i + 1];
			// printf("**** lj %d %d treemark %d \n", li, hb->MutliLevelneighborHBNodesTranspose[si][2*i], treemark);
			break;
		      }
		    }
		    ...
		    for(jj = 0; jj < hb->blength[si]; jj++){
			  ...
			  if(hb->testAvailableHB(sk,sj,li+1) && li < -1){
			    // DEBUG DEBUG DEBUG DEBUG DEBUG
			    // printf("Something wrong with testAvailableHB: (%d, %d) level %d \n",sk,sj,li);
			    // exit(-1);			    
			    temp[jj][kk] = hb->highGet(hb2high[sk],hb2high[sj],li+1);
			    // printf("Temp [%d][%d] %f %f \n",jj,kk,temp[jj][kk],hb->highGet(hb2high[sk],hb2high[sj],li+1) );
			  }
			  else {
			    FMMkk[FMMcount] = kk;
			    FMMsj[FMMcount] = sk;
		    ...
		    for(jj = 0; jj < hb->linklength[currenttremark]; jj++){
			  ...
			  hb->highSet(hb2high[sl],hb2high[sj],li,val);
			  // printf("Row %d %d %d matrix %f val %f \n", ll, hb2high[sl], hb2high[sj], hb->highGet(hb2high[sl],hb2high[sj],li), val );
		       }
		       //The neighbors of sj is the same as those of si
		       //The points in t and neighborHBNodes are for the same
		       // locations in high but on differnt level and in different orders
...
// void FWTd3::Kwblockmatrixrow(int li, int lj, int currenttremark, int nnei, double **temp, int *FMMkk, int *FMMsj, double *eval, int *FMMtee mp, int *t, int *nodes, int *nodeslength, int wavMaxLevel){
...
void FWTd3::KwStore(cs* csKw, int li, int lj, int *nodes, int *nodeslength,int wavMaxLevel){
       ...
       // Debug
       // if (li == lj) printf("Block %d %d \n ",li,lj);

       for(ii = 0; ii < Kwrow_n; ii++){
	 si = hb->waveletNodes[hb->wavlist[ii + hb->wavlevels[li][0]]];
	 // multileveldistcritUpper(lj, si, nodes, nodeslength);
	 MultilevelNeighbor(lj, si, nodes, nodeslength, levelcubes, subdividedlevelcubes);
	 // printf("Column Current Treemark %d Level %d \n",treemark, nei->tree[treemark][11]);
	 // printf("si = %d waveletIndex = %d---------------------------------------- \n",si, hb->waveletIndex[si]);
	 for(jj = 0; jj < *nodeslength; jj++){
	   int sj = nodes[jj];	   
	   if(hb->wavelet[sj]){
	     // Save lower triangular only
	     if(hb->waveletIndex[sj] <=  ii + hb->wavlevels[li][0]){
	     cs_entry(csKw, ii + hb->wavlevels[li][0], hb->waveletIndex[sj], hb->highGet(hb2high[si],hb2high[sj], 
		      li + q*(wavMaxLevel + 1) - (q - 1)*q/2) ); 
	     }
	   }
	 }
       }
...
void FWTd3::MultilevelNeighbor(int lj, int si, int *nodes, int *nodeslength, int *levelcubes, int *subdividedlevelcubes){
	...
	// int* subdividedlevelcubes = new int[maxLength];	

        // printf("Compute j > i distance criterion neighborhoods \n");       	
        cube = hb->cubenode[si];
	int silevel = nei->tree[cube][11];
        precounter = 0;
	// printf("*Cube %d Level %d Cube Length %d ------------------- \n", cube, silevel, hb->neighborHBCubesLength[si]);
	...
	//  printf("Subdivided cubes precounter %d \n", precounter);
	  ...
	  for(int q = 0; q < precounter; q++){
		...
		// printf("%d ", subdividedlevelcubes[counter]);
		counter++;
	      }
	    }
	  }
	...
	// Debug
	// printf("\n");
	// printf("*Cube %d Source Level %d Target Level %d Cube Length %d ------------------- \n", cube, silevel, lj, totalcounter);
        // Output the nodes and nodes length through pointers
