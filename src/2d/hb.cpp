...
HB::HB(int treesize, int n, int van){
	...
	///for debug only
/*	int maxSize = 6000;
	DavailableHB = new int*[maxSize];
	for(int i = 0; i < maxSize; i++){
		DavailableHB[i] = new int[maxSize];
		for(int j = 0; j < maxSize; j++) 
			DavailableHB[i][j] = 0;
	}
	Dhigh = new double*[n];
	for(int i = 0; i < maxSize; i++){
		Dhigh[i] = new double[n];
		for(int j = 0; j < n; j++)
			Dhigh[i][j] = 0;
	}
*/
...
double HB::highGet(int si, int sj, int level){
	...
	if (Ai[ii] == si && A->p[sj] < A->p[sj+1]){
	  // if(A->x[ii] == 0)
	  //			printf("highGet: high(%d, %d) = 0.0 \n",si,sj);///
		return A->x[ii];
	}else {
	  // printf("highGet: high(%d, %d) = 0 \n",si,sj);///
		return 0;
	}
}
