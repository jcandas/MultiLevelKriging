...
PetscErrorCode gmres_init(int argc, char* argv[], int n, int numCenters, int gridSize, double alpha, int d, int interpFuncExtent, double tol, int max_iter,int van, int pvan,double*rbfCenterPositions, fftw_complex* RBF3D, KSP& solver, Mat& mA, Mat& mD, int minlevel, double* b){
	...
	printf("gmres_init: FWTd3 initializaed.\n");///
	...
	*/
	...
	int wn = ctx->fwtd3->hb->waveletSize;
		...
		writeMatrix(fname,n-wn,n,vv1);
	        //	printf("TESTING: vv1 written to file\n");
		for(int i = 0; i < n-wn; i++)
			delete[] vv1[i];
		delete[] vv1;

        if( !getNormalizedPositions( rbfCenterPositions, ctx->xk, ctx->yk, ctx->zk, ctx->rbfWidth, gridSize, numCenters, ctx->interpFuncExtent, ctx->numFreq ) )
        {
...
PetscErrorCode PMult(PC pc, Vec x, Vec b){
	...
	for(int level = maxLevel; level >= 0; level--){
		...
		//read out the rhs at this level
		if(rank == 0){
...
// temp[N : 2N-1] pointers from node list to support index
	...
	double alpha  = hb->kernelParameter;
	double a2     = sqrt(2 * hb->kernelParameter);	
	double mid = 1e-8;
	// double K0;
